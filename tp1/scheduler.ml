...
let read_exp (id, ex) = 
  ...
  | Ereg(_) -> ()
  | Enot(a) -> read_arg a
  | Ebinop(op, a1, a2) -> read_arg a1 ; read_arg a2
  | Emux(a1, a2, a3) -> read_arg a1 ; read_arg a2 ; read_arg a3
  | Erom(_, _, a) -> read_arg a
  | Eram (_, _, a1, a2, a3, a4) -> read_arg a1 ; read_arg a2 ; read_arg a3 ; read_arg a4
...
let schedule p = 
  ...
  (* ajout des arêtes *)
  List.iter (fun (output, eq) -> 
    List.iter (fun input -> if input != output then add_edge g input output) (read_exp (output, eq))
  ) p.p_eqs;
  ...
  let filtered_list = List.filter (fun ident -> Hashtbl.mem variables_with_eq ident) sorted_eq in
  try
  {   
    p_eqs =
      (* correspondance idents <-> equations *)
      List.map (fun ident -> Hashtbl.find variables_with_eq ident) filtered_list;
    p_inputs = p.p_inputs;
